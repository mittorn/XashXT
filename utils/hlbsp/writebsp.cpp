...
typedef std::map<std::pair <int, std::pair<short, short>>, int> clipnodemap_t;
...
void            FinishBSPFile()
{
	...
	if (!g_nobrink)
	{
		Log ("FixBrinks:\n");
#ifdef ZHLT_XASH
		dclipnode_t clipnodes[MAX_MAP_HULLS - 1][MAX_MAP_CLIPNODES];
		int numclipnodes[MAX_MAP_HULLS - 1];
#else
		dclipnode_t clipnodes[MAX_MAP_CLIPNODES];
		int numclipnodes;
#endif
		void *brinkinfo[MAX_MAP_MODELS][NUM_HULLS];
		int headnode[MAX_MAP_MODELS][NUM_HULLS];
		int i, j, level;
