...
void R_AddGrass( grasspinfo_t *ppinfo )
{
	...
	for( surf = stex->texturechain; surf; surf = surf->texturechain )
	{
		...
		es->grasscount += GRASS_DENSITY( es, ppinfo );
	}
}
...
void R_ConstructGrass( msurface_t *psurf )
{
	...
	for( int i = 0; i < grassInfo.Count(); i++ )
	{
		...
		if( !Q_stricmp( texname, ppinfo->stex ))
		{
			...
			int count = GRASS_DENSITY( es, ppinfo );
...
void R_DrawGrass( int pass )
{
	...
	if( pass != GRASS_PASS_LIGHT && pass != GRASS_PASS_FOG )
	{
        ...
        glTexCoordPointer( 2, GL_SHORT, sizeof( grassvert_t ), grassverts->t );
	}

	if( pass == GRASS_PASS_NORMAL || pass == GRASS_PASS_AMBIENT )
	{
        glEnableClientState( GL_COLOR_ARRAY );
        glColorPointer( 3, GL_UNSIGNED_BYTE, sizeof( grassvert_t ), grassverts->c );
	}

	GL_Cull( GL_NONE );
    glAlphaFunc( GL_GEQUAL, bound( 0.0f, r_grass_alpha->value, 1.0f ));

    glEnableClientState( GL_VERTEX_ARRAY );
    glVertexPointer( 3, GL_FLOAT, sizeof( grassvert_t ), grassverts->v );
