	...
	for( p = polys; p; p = p->next )
	{
		...
		for( i = 0, v = p->verts[0]; i < p->numverts; i++, v += VERTEXSIZE )
		{
			if( reflection )
			{
                ...
                glMultiTexCoord3f( GL_TEXTURE1_ARB, stw.x, stw.y, stw.z );
...
void EmitWaterPolysReflection( msurface_t *fa )
{
    ...
    glBegin( GL_POLYGON );
    glMultiTexCoord3f( GL_TEXTURE3_ARB, fa->plane->normal.x, fa->plane->normal.y, fa->plane->normal.z );
	...
	for( int i = 0; i < mesh->numVerts; i++ )
	{
        ...
        glMultiTexCoord3f( GL_TEXTURE1_ARB, stw.x, stw.y, mesh->verts[i].vertex.z );
        glMultiTexCoord3f( GL_TEXTURE2_ARB, dir.x, dir.y, dir.z );
