...
static void R_Bloom_GeneratexDiamonds( void )
{
    ...
    glScissor( 0, 0, sample_width, sample_height );
    ...
    glScissor( 0, 0, glState.width, glState.height );
...
static void R_Bloom_DownsampleView( void )
{
	...
	if( r_screendownsamplingtexture_size )
	{
        ...
        glCopyTexSubImage2D( GL_TEXTURE_2D, 0, 0, 0, curView_x, glState.height - ( curView_y + curView_height ), curView_width, curView_height );
...
void R_BloomBlend( const ref_params_t *fd )
{
    ...
    glCopyTexSubImage2D( GL_TEXTURE_2D, 0, 0, 0, 0, 0, r_screenbackuptexture_width * sampleText_tcw, r_screenbackuptexture_height * sampleText_tch );
