...
void DrawRain( void )
{
    ...
    glTexCoordPointer( 2, GL_FLOAT, 0, m_coordsarray );
	...
	if( Rain.weatherMode == MODE_RAIN )
	{
		while( Drip != NULL )
		{
			...
			{
				// it's time to flush
                glDrawElements( GL_TRIANGLES, m_iNumIndex, GL_UNSIGNED_SHORT, m_indexarray );
				m_iNumVerts = m_iNumIndex = 0;
			}
	...
	{ 
		while( Drip != NULL )
		{
			...
			{
				// it's time to flush
                glDrawElements( GL_QUADS, m_iNumIndex, GL_UNSIGNED_SHORT, m_indexarray );
				m_iNumVerts = m_iNumIndex = 0;
			}
