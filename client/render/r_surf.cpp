...
static void LM_UploadBlock( qboolean dynamic )
{
	...
	else
	{
        ...
        glTexSubImage2D( GL_TEXTURE_2D, 0, 0, 0, BLOCK_SIZE, BLOCK_SIZE, GL_RGBA, GL_UNSIGNED_BYTE, gl_lms.lightmap_buffer );
...
void R_DrawProjectDecal( struct decal_s *pDecal, struct msurface_s *fa, float *v, int numVerts )
{
	...
	{
        glMultiTexCoord2f( GL_TEXTURE3_ARB, v[3], v[4] );
        glVertex3fv( v );
	}
...
void R_DrawWaterSurfaces( bool fTrans )
{
	...
	if( fTrans )
	{
        ...
        glBlendFunc( GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA );
        glTexEnvi( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE );
        glColor4f( 1.0f, 1.0f, 1.0f, RI.refdef.movevars->wateralpha );
	}
	else
	{
        glDisable( GL_BLEND );
