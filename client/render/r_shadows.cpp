...
int R_AllocateShadowTexture( void )
{
    ...
    glCopyTexImage2D( GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, RI.viewport[0], RI.viewport[1], RI.viewport[2], RI.viewport[3], 0 );
...
void R_RenderShadowmaps( void )
{
	...
	for( int i = 0; i < MAX_PLIGHTS; i++ )
	{
		...
		{
			// allow screen size
			RI.viewport[2] = bound( 96, RI.viewport[2], width );
			RI.viewport[3] = bound( 72, RI.viewport[3], height );
		}
		else
		{
